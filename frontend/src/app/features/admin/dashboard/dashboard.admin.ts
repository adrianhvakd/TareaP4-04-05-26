...
export class DashboardAdminComponent implements OnInit {
    ...
    pagosDoughnutLabels: string[] = ['Pendiente', 'Cobrado'];
    pagosDoughnutData: number[] = [0, 0];
    ...
    updateCharts() {
        ...
        const montoPorMes: number[] = new Array(12).fill(0);
        
        this.tramites().forEach(t => {
            if (t.fechaCreacion) {
                const fecha = new Date(t.fechaCreacion);
                const mes = fecha.getMonth();
                montoPorMes[mes] += Number(t.monto);
            }
        });

        this.barChartLabels = meses;
        this.barChartData = montoPorMes;
    }

    updatePagosChart() {
        const pendiente = this.montoTotal() - this.montoCobrado();
        this.pagosDoughnutData = [Math.max(0, pendiente), this.montoCobrado()];
    }
